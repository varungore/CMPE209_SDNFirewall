...
class LearningSwitch (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    #print 'Pushing Rules'
    policyFile = "%s/py_scripts/firewall/policies.csv" % os.environ[ 'HOME' ]
    deny = []
    with open(policyFile, 'rb') as f:
        reader = csv.DictReader(f)
        for row in reader:
            deny.append((EthAddr(row['mac_0']), EthAddr(row['mac_1'])))
            #self.deny.append((EthAddr(row['mac_1']), EthAddr(row['mac_0'])))

    #Firewall.update_policies        
    for (src, dst) in deny:
        match = of.ofp_match()
	match.dl_src = src
	match.dl_dst = dst
	...
	self.connection.send(msg)
	"""# Forward every packet to our firewall 
	msg_rep = of.ofp_flow_mod()
        msg_rep.match = of.ofp_match.from_packet(packet, event.port)
        msg_rep.idle_timeout = 5
        msg_rep.hard_timeout = 15
        msg_rep.actions.append(of.ofp_action_output(port = port))
        msg_rep.data = event.ofp # 6a
